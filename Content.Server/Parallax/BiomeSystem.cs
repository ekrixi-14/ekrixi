            ...
            Parallel.ForEach(chunks, new ParallelOptions() { MaxDegreeOfParallelism = _parallel.ParallelProcessCount }, chunk =>
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    while (groupCount > 0 && frontier.Count > 0)
                    {
                        ...
                        TryGetEntity(node, component.Layers, noiseCopy, grid, out var proto, out var _);
        ...
        EntityQuery<TransformComponent> xformQuery)
    {
        ...
        for (var x = 0; x < ChunkSize; x++)
        {
            for (var y = 0; y < ChunkSize; y++)
            {
                ...
                if (anchored.MoveNext(out _) || !TryGetEntity(indices, component.Layers, noise, grid, out var entPrototype, out var entAnchored))
                    continue;
                ...
                if (xformQuery.TryGetComponent(ent, out var xform) && !xform.Anchored && entAnchored)
                {
